...
int demander_outil(int outil_id) {
    ...
    if (strncmp(response, "ok", 2) == 0) {
        printf("[comm] Bras %d: outil %d recu.\n", bras_id, outil_id);
        return 1;
    }
    return 0;
}
int demander_deux_outils(int id1, int id2) {
    ...
    recv(sock, response, sizeof(response), 0);
    if (strncmp(response, "ok", 2) == 0) {
        printf("[comm] bras %d: outils %d et %d recus.\n", bras_id, id1, id2);
        return 1;
    }
    return 0;
}
void liberer_outil(int outil_id) {
    ...
    send(sock, msg, strlen(msg), 0);
    printf("[assemblage] bras %d: outil %d libere.\n", bras_id, outil_id);
}
...
void *thread_comm(void *arg) {
    ...
    while (1) {
        ...
        while (!succes && tentative < 3) {
            ...
            if (!succes) {
                printf("[comm] bras %d: echec tentative %d, ressai...\n", bras_id, tentative+1);
                sleep(1);
                tentative++;
            }
        }
        if (succes) {
            ...
            if (strncmp(sync_response, "sync_ok", 7) == 0) {
                printf("[comm] bras %d: synchronisation reussie\n", bras_id);
            }
            sleep(1); 
        } else {
            printf("[comm] bras %d: abandon apres 3 tentatives\n", bras_id);
            sleep(5); 
        }
    }
    return NULL;
...
void *thread_assemble(void *arg) {
    while (1) {
        printf("[assemblage] bras %d: tache en cours avec outils %d et %d...\n", 
...
int main(int argc, char *argv[]) {
    ...
    printf("bras %d connecte au serveur (outils %d et %d).\n", 
          bras_id, outil1_id, outil2_id);
