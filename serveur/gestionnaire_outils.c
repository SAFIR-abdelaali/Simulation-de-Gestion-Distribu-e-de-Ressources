...
int current_strategy = STRATEGIE_WAIT_DIE; // Default strategy
...
// Add log entry with thread-safe protection
...
// Wait-die strategy implementation
...
int acquerir_outils(int bras_id, int id1, int id2, int client_socket) {
    ...
    if (!success) {
        ...
        snprintf(log_msg, sizeof(log_msg), "rejeté (stratégie %s)", 
...
void liberer_outil(int bras_id, int outil_id, int client_socket) {
    ...
    ajouter_log(bras_id, "outil %d libéré", outil_id);
    send(client_socket, "ok\n", 3, 0);
}
...
void changer_strategie(int bras_id, const char *strategy, int client_socket) {
    if (strcmp(strategy, "wait-die") == 0) {
        ...
        ajouter_log(bras_id, "stratégie changée en wait-die");
        send(client_socket, "strategie wait-die\n", 19, 0);
    } else if (strcmp(strategy, "wound-wait") == 0) {
        current_strategy = STRATEGIE_WOUND_WAIT;
        ajouter_log(bras_id, "stratégie changée en wound-wait");
        send(client_socket, "strategie wound-wait\n", 20, 0);
    } else {
        ajouter_log(bras_id, "stratégie inconnue: %s", strategy);
...
void *gerer_client(void *arg) {
    ...
    close(client_socket);
    ajouter_log(bras_id, "déconnecté");
    return NULL;
}
